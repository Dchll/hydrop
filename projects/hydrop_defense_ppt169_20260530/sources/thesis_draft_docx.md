...
本系统选择 Flutter 的主要原因是项目同时面向移动端和桌面端，并且界面需要在不同窗口宽度下保持统一风格。若分别使用 Android 原生、iOS 原生和桌面端原生技术开发，设备发现、聊天页、设置页和文件传输进度界面需要重复实现多套，毕业设计阶段的维护成本较高。Flutter 可以用同一套 Dart 代码覆盖主要平台，并通过 Material 组件实现黑白灰、直角、2px 分割线等统一视觉风格。系统虽然模仿桌面和移动端常见即时通信软件的布局，但不依赖某个平台专有控件，因此 Flutter 的自绘能力更适合该项目。
...
没有选择 Web 技术作为主实现，是因为本系统需要访问本地文件、监听局域网端口、显示系统通知并处理下载目录保存，普通浏览器环境对这些能力存在明显限制。没有选择纯原生多端开发，是因为功能重复实现过多，且 UI 风格难以在各端保持一致。Flutter 在平台能力方面可以通过插件补齐文件选择、通知、路径和二维码能力，同时保留统一的 UI 代码，因此更符合本系统的开发目标。
...
Riverpod 在本系统中还承担依赖注入的作用。设备仓库、地址仓库、消息仓库、设置仓库、发现控制器、传输协调器等对象通过 Provider 暴露，页面不需要手动创建这些对象，也不需要关心它们的生命周期。以聊天页为例，页面只需要根据 remoteDeviceId 读取会话消息和传输进度，再调用 ChatPageController 完成发送、选择文件、删除消息等操作。底层到底是读取 Drift、建立 TCP 连接，还是调用 file_picker，都被封装在应用层和基础设施层中。
...
本系统选择 Riverpod，而不是只使用 StatefulWidget 或 InheritedWidget，原因在于项目状态来源较多，且状态不完全属于某个页面。设备发现由运行时服务产生，消息由 TCP 接收端或用户输入产生，传输进度由文件协调器产生，设置项需要被 MainApp、设置页和运行时共同读取。若全部放在页面局部状态中，页面销毁后状态容易丢失，也会使网络服务和 UI 生命周期绑定过紧。Riverpod 可以把这些状态提升到应用层，并通过 Provider 精确控制依赖关系。
...
Riverpod 的另一个选型原因是便于测试和替换实现。设备列表、会话消息和设置项都可以通过 Provider 暴露，测试或调试时可以替换仓库或状态来源。虽然本文不把大量测试代码放入正文，但从工程结构看，Provider 化的依赖比全局单例更容易控制，也更适合后续增加自动化测试。
...
本系统选择 AutoRoute 的原因是聊天页需要强类型参数。ChatRoute 必须携带 remoteDeviceId 和 displayName，如果使用字符串路径手动拼接参数，容易出现参数遗漏、名称写错或类型不一致的问题。AutoRoute 通过生成代码在编译期约束路由参数，缺少必要参数时会直接暴露错误，这比运行时进入页面后再发现上下文为空更可靠。
...
本系统选择 Drift 和 SQLite，而不是 Hive、SharedPreferences 或单纯文件存储，是因为项目数据具有明显的关系结构。远端设备和设备地址是一对多关系，消息和附件也是一对多关系，连接会话、设置项、本机资料和统计信息又需要分别保存。如果使用键值存储，查询某台设备的消息、某条消息的附件、某个地址的可达状态会变得零散；如果使用 JSON 文件，重复写入和并发更新也不易处理。SQLite 适合保存结构化关系数据，Drift 则为 Dart 项目提供类型安全的表定义、查询和 Stream 监听能力。
Drift 的响应式查询与本系统 UI 模型比较契合。设备发现控制器写入设备表后，设备页可以自动刷新；接收端写入消息表后，聊天页可以自动出现新消息；设置页修改主题或语言后，MainApp 可以读取设置并更新界面。若直接使用 sqlite3 手写 SQL，也能实现这些功能，但需要手动维护查询结果到 UI 的同步。Drift 把表结构、DAO、数据类和 Stream 查询整合到一起，减少了这部分重复工作。
数据库版本保持 schemaVersion = 1 的原因也与项目阶段有关。系统处于第一版开发阶段，开发机上的旧数据可以通过清理应用数据重新创建，不需要为频繁变化的表结构维护复杂迁移。等系统进入稳定发布阶段，再引入从旧版本到新版本的迁移脚本更合适。论文中按当前代码描述 schemaVersion = 1 和首次创建数据库，可以避免把尚未实现的迁移能力写成已完成内容。
...
FrameCodec 的设计重点是解决 TCP 字节流边界问题。TCP 面向字节流，不保证一次读取正好对应一次应用层消息；如果直接用换行符或字符串拼接协议，遇到二进制文件内容时容易出现边界不清或转义复杂的问题。因此系统在每个帧前放置 header 长度和 body 长度，接收端先读取长度，再读取对应数量的 header 和 body。header 使用 JSON 保存帧类型、协议版本、消息 ID、附件 ID、文件名、偏移量等元数据，body 用于存放文本内容或文件分片二进制数据。这样既保留了元数据的可读性，也减少了文件内容编码转换的开销。
设备发现选择 UDP 广播，是因为发现阶段只需要让局域网中的设备知道“我在这里”，不需要为每台未知设备预先建立连接。UDP 广播实现成本低，适合周期性发送较小的公告 payload。即使少量广播丢失，下一轮广播仍可刷新设备状态。若发现阶段使用 TCP，系统必须知道对端地址才能连接，这与“自动发现附近设备”的目标相冲突。若依赖中心服务器，又会失去局域网直传不经过外网的特点。
数据传输选择 TCP，是因为文本消息和文件内容需要可靠、有序地送达。文件分片如果乱序、丢失或重复，都需要额外处理；TCP 已经提供可靠传输、顺序保证和拥塞控制基础，适合作为文件传输的底层协议。系统在 TCP 之上仍然设计 fileOffer、fileChunk、fileComplete 和 ACK，是为了表达应用层状态，例如接收端是否接受文件、某个附件传输了多少、完成后是否校验成功。这些语义不是 TCP 自身能直接提供的。
...
文件分片大小选择 1MB，是在传输效率和内存占用之间折中。分片过小会增加帧数量和 ACK 次数，降低大文件传输效率；分片过大则会增加单次内存占用和错误重试成本。1MB 分片可以减少帧数量，同时避免一次性占用过多内存。最大文件大小设置为 16GB，是为了防止用户误选极端大文件造成磁盘、内存和长时间连接风险；并发传输上限设置为 2，是为了避免多个大文件同时争抢网络和磁盘资源。
...
选择 freezed 的原因是状态模型经常需要不可变复制。传输进度快照、消息状态和设置状态在 UI 中频繁变化，如果直接修改同一个可变对象，页面是否重建、状态是否被意外覆盖都不容易判断。不可变对象配合 copyWith 可以清楚表达“基于上一状态生成下一状态”的过程，也更适合 Riverpod 的状态更新方式。虽然不可变建模会增加一些生成代码，但可以减少手写相等判断和复制逻辑。
...
设备发现功能需要满足“自动、可恢复、不过度打扰”的要求。自动是指应用进入前台后能主动发送广播并监听对端广播；可恢复是指短时间网络抖动不会立即删除设备，而是通过 TTL 将地址标记为不可达；不过度打扰是指发现过程不应频繁弹窗或要求用户确认。设备页只展示设备名称、速度摘要、最后一次传输和在线状态，不在线设备通过灰色文本区分。这样用户可以快速判断哪些设备可用，而不需要阅读底层网络日志。
文本消息功能需要满足“可见、可追踪、可失败”的要求。用户发送文本后，消息应立即进入本地时间线，而不是等待网络确认后才出现；消息发送过程中需要有 pending 或 sending 状态，确认成功后进入 sent，失败后进入 failed。失败消息不应直接消失，因为消失会让用户无法判断是没有点击成功，还是网络发送失败。保留失败状态也便于后续加入重试、复制错误信息或诊断提示。
文件传输功能需要考虑大文件场景。系统应避免使用一次性 readAsBytes 的方式读取大文件，而应按分片流式读写；接收端应把文件保存到下载目录下的 Hydrop 文件夹，方便用户在系统文件管理器中找到；传输过程中应展示速度、已传字节、总字节和阶段状态。对于暂停、取消和失败，UI 应把状态落到具体附件消息上，而不是只显示一个全局提示。这样用户可以在聊天记录中回看某个文件是否成功传输。
设置管理功能需要和系统运行时关联。主题和语言属于界面偏好，修改后应通过 Provider 推送到 MainApp 并即时生效；本机显示名称属于发现 payload 的一部分，修改后会影响对端设备列表中的名称；自动恢复传输开关影响应用启动时是否尝试恢复未完成任务；传输加密开关虽然在当前阶段主要作为配置入口保留，但它在数据库中已有字段，便于后续接入真正的加密传输。
...
非功能需求还包括可观察性。局域网传输问题往往不容易复现，可能与网络隔离、路由器广播限制、系统权限、端口占用或对端应用状态有关。如果系统没有日志，开发者只能通过界面现象猜测问题位置。当前项目在消息发送链路中使用中文日志和统一标志符记录关键错误，使开发者可以区分地址选择失败、TCP 建连失败、确认超时、文件过大和通知初始化失败等不同问题。日志不直接面向普通用户，但它能显著提高开发和调试效率。
...
从代码组织角度看，系统尽量避免让页面文件承担过多业务职责。设备页只关心设备列表和选中设备，聊天页只关心远端设备会话，设置页只关心本机资料和偏好配置。文本发送、文件选择后的消息创建、传输进度更新、设备删除、设置保存等行为都通过 Controller 或 Provider 暴露的方法完成。这样在修改 UI 风格时，不需要重写发现协议和传输协议；在修改传输策略时，也不需要重新设计页面结构。
...
传输模块的可维护性重点在于把“任务控制”和“字节传输”分开。用户看到的是附件消息的暂停、取消、失败、完成和速度；底层实际执行的是文件读取、Socket 写入、分片确认和状态回写。FileTransferCoordinator 把这两部分连接起来，但页面不直接接触 Socket，数据库也不直接管理 Socket 生命周期。后续若要增加传输队列、并发优先级、断点续传或加密流包装，应优先在传输协调器和连接服务中处理，而不是把逻辑分散到聊天页。
设置模块的扩展也需要保持边界清晰。主题和语言由 MainApp 消费，传输加密和自动恢复由运行时或传输模块消费，本机显示名称由发现 payload 消费。设置页只是这些状态的编辑入口，不应把设置值写死到页面内部。通过设置仓库和 Provider 暴露设置状态，系统可以在用户切换语言时立即刷新界面，在修改显示名称后让后续广播携带新的名称，在切换自动恢复时影响启动恢复逻辑。
系统的错误信息需要面向两类对象：开发者和普通用户。开发者需要日志中包含足够定位信息，例如远端设备 ID、本地消息 ID、附件 ID、文件名和错误类型；普通用户只需要知道“发送失败”“文件过大”“连接中断”“对端不可达”等可理解结论。因此应用层记录详细日志，页面层展示简洁状态。这样既不会让用户界面被技术细节淹没，也不会让开发调试时缺少必要线索。
可维护性还要求文档、论文和代码保持一致。本文只把设备页和设置页作为一级入口，不再描述独立传输页或我的页；只描述 schemaVersion = 1 和首次创建数据库，不再描述多版本迁移；只描述当前文件传输协议参数，不写未经实测的吞吐量结论。这样做可以减少论文和程序行为之间的矛盾，也便于后续按真实代码继续维护毕业设计材料。
...
通知权限也放在运行时处理，而不是放在文件发送按钮中处理。传输通知是系统级反馈能力，适合在应用启动阶段提前申请或检查；真正发送文件时，只需要调用通知服务更新进度。如果通知插件缺失、权限不足或平台不支持，运行时会把错误控制在通知模块内，不让异常影响文件传输主流程。这个处理方式在桌面端尤其重要，因为 macOS 通知权限和插件注册状态可能与移动端不同。
中断传输恢复由 AppRuntime 触发，说明恢复行为属于应用级能力。系统启动后可以检查数据库中处于 transferring、pending 或 paused 等未完成状态的附件记录，并根据自动恢复设置决定是否尝试恢复。恢复过程仍然经过 FileTransferCoordinator，而不是由页面扫描数据库后自行恢复。这样即使用户先打开设置页或设备页没有选中某个聊天，会话中未完成的任务也可以由运行时统一处理。
...
路由结构刻意保持较少层级。设备页和设置页是用户进入应用后最常用的两个入口，因此放入 AppRoute 的子路由中；聊天页虽然是核心功能页面，但它必须依赖远端设备上下文，不能作为没有参数的一级 tab 出现。ChatRoute 要求 remoteDeviceId 和 displayName 两个参数，前者用于查询消息、附件和传输进度，后者用于标题展示和空状态提示。通过参数强制约束，可以避免进入聊天页却不知道对应设备的情况。
响应式主界面通过窗口等级统一处理，而不是在每个页面中零散判断屏幕宽度。compact 和 medium 宽度使用底部导航，符合手机端用户手指操作习惯；expanded 和 large 使用左侧导航，给内容区域留下更多垂直空间。设备页在桌面端内部再拆成设备列表和聊天内容两列，其中设备列表宽度固定在适合扫描的范围内，聊天内容占据剩余区域。这样设备页在桌面端接近即时通信软件的布局，在移动端仍保持清晰的页面栈。
导航项被压缩为设备和设置两个，也减少了状态同步成本。独立传输页虽然可以集中展示任务，但会让文件传输从聊天上下文中抽离出来；我的页如果独立存在，也会让本机名称、二维码和偏好设置分散。当前实现把传输状态放回附件消息，把本机资料合并到设置页，使用户在两个入口之间即可完成主要操作。这与系统面向局域网设备直传的定位更一致。
主界面在视觉上使用黑白灰、直角和内部分割线。页面外缘不再使用明显外框，也不使用每日壁纸或玻璃背景，避免内容被装饰元素干扰。桌面侧边导航、手机底部导航、设备列表和聊天区域之间主要依靠 2px 分割线建立结构关系。这样的风格更接近工具型应用，适合文件传输这种需要关注状态和效率的场景。
...
设备离线不是通过一次连接失败立即判断，而是通过 TTL 过期判断。UDP 广播本身允许丢包，如果一次没有收到广播就把设备置为离线，设备列表会频繁闪烁。系统设置 10 秒广播间隔和 12 秒 TTL，使一次偶发丢包不会立刻影响 UI；TTL 扫描每 3 秒执行一次，能够在设备离开网络后一段时间内把地址标记为不可达。设备是否离线取决于是否仍存在可达地址，而不是简单取最后一次广播时间。
...
文本消息发送从 ChatPageController 发起。用户输入文本后，控制器先创建本地消息记录，使消息立即显示在时间线中；随后根据远端设备 ID 查询可达地址，通过 TCP 连接发送 textMessage 帧；接收端处理文本后返回确认帧。发送端收到确认后把消息状态改为 sent，如果超时或收到错误帧，则把状态改为 failed 并保存错误原因。这个流程保证了消息不会因为网络失败而无声消失。
...
大文件传输实现避免一次性读入内存。发送端通过文件流按 1MB 读取分片，每次读取后立即编码为帧并写入 Socket；接收端收到分片后按 offset 写入目标文件，并更新 transferredBytes。SHA-256 校验也应采用流式思路处理，避免在几 GB 文件场景中把完整文件加载到内存。这样的实现虽然代码比一次性读写复杂，但更符合大文件传输需求，也能降低移动设备内存压力。
暂停和取消属于用户控制能力。暂停传输时，系统需要停止继续发送或接收数据，关闭相关连接，并把附件状态更新为 paused；取消传输时，系统将任务置为 failed 或取消结果，并更新 UI 与通知。暂停并不等同于删除消息，用户仍然可以在聊天记录中看到附件和进度；删除消息也不等同于删除本地文件，尤其是接收到的文件默认保存到下载目录 Hydrop 文件夹，用户可在系统文件管理器中继续访问。
接收端保存路径固定为下载目录下的 Hydrop 文件夹。这样用户接收文件后不需要进入应用私有目录查找，也方便在桌面端或手机文件管理器中直接打开。保存时需要处理文件名冲突，避免同名文件覆盖。系统可以通过追加后缀或生成唯一文件名的方式保证每次接收都有独立路径，再把最终路径写入附件记录。
错误处理贯穿整个传输链路。地址不可达、连接建立失败、fileOfferAck 超时、分片发送失败、完成确认失败、文件超过大小限制、磁盘写入失败和校验失败都需要转换成附件状态和错误信息。页面不展示底层异常栈，而是在附件卡片上显示失败状态；日志保留更详细的中文错误，便于开发阶段定位问题。这样用户可以理解结果，开发者也能追踪原因。
...
速度计算依赖连续两次进度快照之间的字节差和时间差。发送端和接收端都通过同一进度模型暴露速度，因此 UI 可以用同一套组件展示上传和下载状态。对于小文件，进度可能很快从 0 进入完成；对于大文件，进度会持续更新。进度条不应简单显示满格，而应根据 transferredBytes 和 totalBytes 计算百分比；如果 totalBytes 不合法或未知，应显示明确的等待或异常状态，避免给用户造成“已经完成”的误解。
进度持久化需要节流。传输中的每个 1MB 分片都可能产生进度事件，如果每次事件都立即写数据库，长时间大文件传输会造成大量写入。系统通过 Riverpod 进度存储向 UI 高频推送，再按阈值把关键进度写回数据库。完成、失败、暂停和取消属于关键状态，必须及时落库；普通传输进度则可以按字节差或时间间隔写入。这样既能保证 UI 流畅，也能减少 SQLite 写入压力。
通知服务读取同一进度来源，避免“通知进度”和“聊天页进度”各算一套。移动端在传输开始、失败或完成时可以通过 fluttertoast 给出短提示，传输过程则可在系统通知中展示进度；桌面端更适合使用应用内提示，因为用户常在窗口内操作。通知初始化失败不应影响文件发送，这是实现中专门处理的边界：通知只是反馈渠道，不是传输协议的一部分。
进度状态还为恢复和诊断提供依据。附件记录中的 transferredBytes、transferStatus 和 errorMessage 可以在应用重启后帮助判断哪些任务未完成；运行时恢复时可以根据这些字段决定是否继续尝试。即使恢复能力后续进一步完善，当前进度模型也已经把方向、阶段、字节数和错误原因拆开，为后续扩展保留了清晰入口。
...
设备页的核心是设备列表。列表项不显示过多网络字段，而是把用户真正关心的信息放在前面：设备名称、在线状态、速度摘要和最后一次传输。不在线设备通过灰色文本表达，在线设备使用正常文本颜色。长按菜单采用 bottom sheet，保持移动端和桌面端统一的弹出方式。删除设备操作针对设备列表显示记录，删除后设备页不再展示该设备；如果对端再次广播，系统仍可根据发现流程重新写入设备。
桌面端设备页内部采用左设备列表、右聊天内容的结构。当用户选择设备后，右侧直接显示 ChatPage 对应内容；没有选中设备时，右侧显示空状态。移动端不强行在窄屏上塞入双栏，而是点击设备后通过 ChatRoute 进入聊天页。这样的实现避免了手机端内容拥挤，也让桌面端用户可以一边查看设备列表，一边处理当前会话。
聊天页的消息时间线按会话设备过滤。文本消息使用左右气泡区分发送和接收；接收消息背景为黑色，发送消息背景为白色，深色主题下也保持可辨识的反差。图片消息采用内联预览，用户不需要只看文件名；普通文件和视频以附件卡片呈现，卡片中展示文件名、大小、进度、速度和状态。消息的复制、删除、保存等操作收敛到长按或悬停菜单，常态界面只保留内容和必要状态。
聊天输入栏采用方形控件。输入框获得焦点后边框加重，发送按钮根据输入内容状态变化，附件按钮负责触发文件选择。手机端键盘弹起时，页面需要根据 MediaQuery 的 viewInsets 调整底部区域，避免输入栏被键盘遮挡或出现负 padding。所有与消息相关的 bottom sheet 都使用响应式约束，避免 AlertDialog 在桌面和移动端之间出现尺寸计算问题。
设置页把“我的”能力合并进来。本机显示名称可以直接在页面上编辑，减少一个弹窗层级；设备 ID、二维码连接、本地 IP 和诊断信息用于连接和排错；主题、语言、传输加密和自动恢复属于全局配置。桌面端设置页可以使用分区列表加详情区域，移动端则以单列列表和 bottom sheet 展开详情。用户不会再看到单独的 Mine 一级入口，论文和程序都应按设置页整合后的形态描述。
...
Drift 的优势在于类型安全和响应式查询。设备页可以订阅设备仓库暴露的设备列表 Stream，聊天页可以订阅指定 remoteDeviceId 的消息 Stream，设置页可以订阅设置表和本机资料表。数据库变化后，Provider 将变化推送到页面，页面无需手动刷新。这种方式特别适合设备发现和消息接收，因为它们可能来自后台网络事件，而不是用户当前页面点击。
...
附件持久化需要同时服务发送端和接收端。发送端附件记录保存原始文件路径、文件名、总字节、已传字节和发送状态；接收端附件记录保存下载目录 Hydrop 下的目标文件路径、接收进度和保存状态。无论方向如何，附件卡片都通过 attachmentId 关联传输进度，通过 messageId 关联消息气泡。这样的结构使 UI 不需要区分“这是发送附件表”还是“这是接收附件表”，只需根据方向和状态调整展示文本。
唯一索引对局域网广播场景很重要。自动发现会持续收到广播，同一设备同一地址可能在几分钟内写入多次；如果没有唯一约束，设备地址表会产生大量重复记录。系统使用设备 ID、IP 地址和端口作为地址唯一条件，并在仓库层使用 upsert 思路更新 lastSeenAt、isReachable 和速度信息。这样既保留地址历史，又不会因为重复广播造成列表膨胀。
外键约束在数据库打开前启用。消息附件依赖消息，设备地址依赖设备，连接会话可能关联设备地址。启用外键约束可以减少孤立记录，但实际业务中仍需注意删除语义。例如删除设备显示记录时，需要考虑是否同时处理关联地址和会话；删除消息记录时，需要处理附件记录，但默认不删除文件系统中的真实文件。这些策略不应只依赖数据库级联，而应由 Repository 根据业务意图明确执行。
...
设备发现测试还需要覆盖多网卡和重复广播场景。在桌面端同时存在 Wi-Fi、虚拟网卡或热点网络时，系统应能从可用 IPv4 地址中构造地址列表，并将远端地址写入 device_address_items。重复广播不会在设备地址表中产生大量重复记录，lastSeenAt 和可达状态应随广播刷新。若路由器限制广播，可通过二维码连接入口验证基本连接信息是否能够补充自动发现能力。
聊天页面测试需要覆盖移动端和桌面端两种布局。在移动端，点击设备后应进入单独聊天页，底部输入栏在键盘拉起时不应被遮挡；在桌面端，设备列表和聊天内容应并排显示，选择不同设备时右侧会话内容随 remoteDeviceId 切换。长按消息或附件时，操作入口应通过 bottom sheet 或悬停菜单出现，常态界面不应长期显示复制、删除等按钮。
文件传输测试除了验证成功路径，还应验证异常路径。选择超过 16GB 的文件时，系统应拒绝发送并显示友好错误；传输过程中关闭对端应用或断开网络时，发送端和接收端都应进入失败或暂停状态，而不是一直显示加载；用户点击暂停后，附件卡片应停止继续增加进度；用户删除文件消息时，只删除消息和附件记录，默认不删除已经保存到下载目录 Hydrop 文件夹中的真实文件。
设置测试需要关注设置项与其他模块之间的联动。修改本机显示名称后，对端设备页后续收到广播时应显示新的名称；切换主题后，设备页、聊天页、设置页和 bottom sheet 的背景、文本、边框颜色都应同步变化；切换语言后，导航项、设置项、聊天输入提示和错误提示应随语言配置更新。自动恢复传输开关需要在应用启动恢复逻辑中生效，传输加密开关则作为配置项持久化保存。
...
工程测试不应只看界面能否打开，还需要检查模块边界是否被破坏。例如页面层不应直接创建 RawDatagramSocket 或 ServerSocket，聊天页不应直接写 Drift 表，设置页不应绕过设置仓库直接修改数据库。若这些边界被破坏，短期内功能可能仍能运行，但后续修改协议、数据库或 UI 时会变得困难。因此工程测试需要结合代码审查，确认 Controller、Repository、Service 和页面之间的依赖方向仍然清晰。
数据库验证还需要关注唯一约束。设备地址表应能处理相同 deviceId、ipAddress 和 port 的重复写入，消息表应避免相同 localMessageId 重复插入，附件表应避免相同 attachmentId 重复插入。此前开发过程中出现过重复列和唯一约束相关问题，这说明数据库设计与写入策略必须配合验证。由于系统不保留迁移逻辑，测试时应使用全新数据库进行创建验证，避免历史开发数据干扰结论。
...
平台能力验证需要分别在移动端和桌面端执行。file_picker 在不同平台上的文件路径、权限和取消行为存在差异，flutter_local_notifications 在 macOS、Android 和 iOS 上的初始化方式也不同。测试时应确认文件选择取消不会创建无效消息，通知初始化失败不会中断传输，移动端 toast 和桌面端应用内提示能按平台分支工作。二维码扫描能力也需要考虑设备是否具备摄像头和权限。
...
大文件传输的性能风险主要来自内存、磁盘和网络三个方面。内存风险来自一次性读取完整文件或一次性保存完整接收内容；磁盘风险来自高频进度落库和接收端写入速度不足；网络风险来自 Wi-Fi 抖动、路由器隔离、对端应用进入后台和 TCP 连接中断。本系统用流式分片读写降低内存风险，用进度节流降低数据库写入压力，用失败状态和错误信息降低无反馈等待风险。由于不同路由器和设备存储速度差异较大，论文不应写固定传输秒数，而应描述验证方法和设计依据。
稳定性分析还需要关注长时间任务。几个 GB 的视频文件传输可能持续较久，期间用户可能锁屏、切换应用、移动设备或关闭对端应用。系统必须在这些情况下尽快把状态反映到 UI，而不是让附件卡片一直保持满进度或加载状态。传输进度快照中的 updatedAt、phase 和 errorMessage 可以帮助判断任务是否仍在推进；心跳和确认超时则用于发现连接失效。后续若要进一步接近生产环境，还可以加入更细的队列状态、重连次数、失败原因分类和可视化诊断信息。
在千兆局域网目标下，单纯增大分片大小并不能解决全部问题。吞吐量还受到 Dart 编码开销、Socket 写入策略、磁盘读写速度、进度刷新频率和 UI 重建范围影响。1MB 分片是性能和内存之间的折中，有限并发 2 可以避免多个大文件互相抢占资源。若后续要继续优化，应通过基准测试分别测量纯文件读取速度、FrameCodec 编解码速度、Socket 传输速度、接收端写盘速度和 UI 刷新开销，再决定是否调整分片大小、引入连接复用或改进缓冲区管理。
